...
if ($userRole == 1) {
    ...
    class articleFunction extends connectDB
    {
        ...
        {
            $id_user = $_SESSION['userid'];
            $sql = "SELECT * FROM articles where user_id = $id_user  ORDER BY id ASC";
            return $this->conn->query($sql);
        }
        ...
        {
            $id_user = $_SESSION['userid'];
            $sql = "SELECT * FROM articles  where user_id = $id_user  ORDER BY `articles`.`create_at` DESC $limit";
            return $this->conn->query($sql);
        }
    }
    $articleFn = new articleFunction();
                    ...
                    <div class="d-flex justify-content-between">
                        ...
                        <div>
                            <a href="article-create.php" class="btn btn-primary btn-icon-split">
                                ...
                                <span class="text">สร้างบทความ</span>
                            </a>
                        </div>
                    </div>
