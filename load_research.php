...
if (isset($_POST['lastid'])) {
    ...
    $sqlAllArticle = "SELECT * ,articles.id as id FROM articles  left join user on articles.user_id = user.id WHERE articles.category_id = $category_name  AND articles.status = 0 AND articles.id > '$lastid' ORDER BY articles.id asc limit 9";
...
?>
        ...
        <div id="remove">
            ...
            <div class="row">
                <div class="col-lg-12">
                    <button type="button" name="loadmore" id="loadmore" id="<?php echo $lastid; ?>" data-cate="<?php echo $Ncategory; ?>" class="btn-btn-success">ดูเพิ่มเติม</button>
                </div>
            </div>
        </div>
<?php
